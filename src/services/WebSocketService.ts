...
class WebSocketService {
  ...
  public connect(
    ...
    const socket = new window.SockJS(
      import.meta.env.VITE_DEV_URL + "/ws/chat?token=" + this.token
    );
    ...
    this.stompClient.connect(
      ...
      (frame) => {
        ...
        if (!this.subscriptions[chatId]) {
          this.subscriptions[chatId] = this.stompClient?.subscribe(
            `/topic/chat/${chatId}`,
            callback
          );
        }
    ...
    responseCreate
      .then((result) => {
        if (result.status == 200) {
          ...
          const responseInvite = communicationApi.inviteUserToChat(
            result.data.chatId,
            userId
          );
          responseInvite
    ...
    this.stompClient.connect(
      ...
      (frame) => {
        ...
        if (!this.subscriptions[userId.toString()]) {
          this.subscriptions[userId.toString()] = this.stompClient?.subscribe(
            `/topic/user/${userId}`,
            callback
          );
        }
