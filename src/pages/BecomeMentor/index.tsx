// src/components/CreateMentorDialog.tsx
...
const BecomeMentorDialog: React.FC<CreateMentorDialogProps> = ({ open, onClose }) => {
    const initialValues: RawMentorData = {
        userId: Number(JSON.parse(atob(localStorage.accessToken.split('.')[1])).sub), // Получаем userId из localStorage
        shortAboutMe: '',
        longAboutMe: '',
        specializations: '',
    };
    ...
    const handleSubmit = async (values: RawMentorData) => {
        const specializationsArray = values.specializations.split(',').map((item: string) => item.trim()).filter((item: string) => item !== '');
